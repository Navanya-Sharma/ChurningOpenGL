...
LightingAssignment::LightingAssignment() :
	m_vertArr(), m_vertBuff(NULL, 26*9* sizeof(float)),
	m_indBuff(NULL, 36 * sizeof(unsigned int)), 
	m_shader("res/LightingAssignmetObject.shader"),
	light_vertArr(), light_vertBuff(NULL, 8 * 3 * sizeof(float)),
	light_indBuff(NULL, 36 * sizeof(unsigned int)), 
	light_shader("res/LightingAssignmetn.shader"),
	gouraud_shader("res/LightingGourd.shader")
{
}

void LightingAssignment::Init()
{
	float vert[24 * 9]  = {
	...
	m_vertArr.Bind();
	m_vertBuff.Bind();
	m_vertBuff.SetData(vert, 24 * 9 * sizeof(float));
	BufferLayout lay;
	lay.Push<float>(3);
	lay.Push<float>(3);
...
void LightingAssignment::UpdateImGui()
{
	ImGui::Text("Lighting Assignment");
	...
	MVP = proj * view * model;
	gouraud_shader.SetUniformMat4("uMVP", MVP);
	gouraud_shader.SetUniformMat4("uModel", model);
	gouraud_shader.SetUniform4f("uLight", lightColor.x, lightColor.y, lightColor.z, 1.0f);
	...
	gouraud_shader.SetUniform4f("uLPos", xl, yl, zl, 0.0f);
	gouraud_shader.SetUniform4f("uVeiw", x, y, z, 0.0f);

	gRenderer.Draw(m_vertArr, m_indBuff, gouraud_shader);

	model=glm::translate(glm::mat4(1.0f), glm::vec3(xl, yl, zl));
...
void LightingAssignment::Update()
{
	Renderer& gRenderer = Renderer::GetRenderer();
